...
for metric in metrics:
    ...
    global_acc = pd.read_csv("./summarized_{}.csv".format(metric), index_col=["id", "n_class"])
    drift_period_acc = pd.read_csv("./summarized_drifted_{}.csv".format(metric), index_col=["id", "n_class"])
    col_l1 = ["Global"]*14
    ...
    complete_df.to_csv("./{}_paper.csv".format(metric), index=True)
    print (complete_df)
